...
class QuestionModelTests(TestCase):
    ...
    def test_published_date_is_in_future(self):
        ...
        """
        time = timezone.localtime() + datetime.timedelta(hours=23, minutes=59)
        question = Question(pub_date=time)
        self.assertIs(question.can_vote(), False)

    def test_voting_allow_at_pub_date(self):
        """
        can_vote() return True for voting that vote
        at exact same time when question is published.
        """
    ...
    def test_voting_on_time(self):
        """
        can_vote() return True for voting
        after pub_date and before end_date.
        """
        time = timezone.localtime() - datetime.timedelta(hours=23, minutes=59)
        end = timezone.localtime() + datetime.timedelta(hours=23, minutes=59)
        question = Question(pub_date=time, end_date=end)
        self.assertIs(question.can_vote(), True)
...
class QuestionDetailViewTests(TestCase):
    ...
    def setUp(self):
        self.username = "test"
        self.password = "1234"
        self.voter = User.objects.create_user(username="test", password="1234")
